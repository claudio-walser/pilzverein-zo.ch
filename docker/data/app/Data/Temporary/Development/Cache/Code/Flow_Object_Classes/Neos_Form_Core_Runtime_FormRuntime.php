 ...
 * @api
 */
class FormRuntime_Original implements RootRenderableInterface, \ArrayAccess
{
    /**
     * @var FormDefinition
     * @internal
 ...
 * This makes it possible to declaratively define how a form should be rendered.
 */
class FormRuntime extends FormRuntime_Original implements \Neos\Flow\ObjectManagement\Proxy\ProxyInterface {
    ...
    public function __construct()
    {
        ...
        if (!array_key_exists(0, $arguments)) $arguments[0] = \Neos\Flow\Core\Bootstrap::$staticObjectManager->get('Neos\Form\Core\Model\FormDefinition');
        if (!array_key_exists(1, $arguments)) $arguments[1] = \Neos\Flow\Core\Bootstrap::$staticObjectManager->get('Neos\Flow\Mvc\ActionRequest');
        if (!array_key_exists(2, $arguments)) $arguments[2] = \Neos\Flow\Core\Bootstrap::$staticObjectManager->get('Neos\Flow\Http\Response');
        if (!array_key_exists(0, $arguments)) throw new \Neos\Flow\ObjectManagement\Exception\UnresolvedDependenciesException('Missing required constructor argument $formDefinition in class ' . __CLASS__ . '. Note that constructor injection is only support for objects of scope singleton (and this is not a singleton) – for other scopes you must pass each required argument to the constructor yourself.', 1296143788);
        if (!array_key_exists(1, $arguments)) throw new \Neos\Flow\ObjectManagement\Exception\UnresolvedDependenciesException('Missing required constructor argument $request in class ' . __CLASS__ . '. Note that constructor injection is only support for objects of scope singleton (and this is not a singleton) – for other scopes you must pass each required argument to the constructor yourself.', 1296143788);
        if (!array_key_exists(2, $arguments)) throw new \Neos\Flow\ObjectManagement\Exception\UnresolvedDependenciesException('Missing required constructor argument $response in class ' . __CLASS__ . '. Note that constructor injection is only support for objects of scope singleton (and this is not a singleton) – for other scopes you must pass each required argument to the constructor yourself.', 1296143788);
        call_user_func_array('parent::__construct', $arguments);
        if ('Neos\Form\Core\Runtime\FormRuntime' === get_class($this)) {
            $this->Flow_Proxy_injectProperties();
        }

        $isSameClass = get_class($this) === 'Neos\Form\Core\Runtime\FormRuntime';
        if ($isSameClass) {
  ...
  'currentPage' => 'Neos\\Form\\Core\\Model\\Page',
  'lastDisplayedPage' => 'Neos\\Form\\Core\\Model\\Page',
  'hashService' => '\\Neos\\Flow\\Security\\Cryptography\\HashService',
  'flashMessageContainer' => '\\Neos\\Flow\\Mvc\\FlashMessageContainer',
  'renderCallbacks' => 'array<callable>',
);
    ...
    public function __wakeup()
    {
        ...
        $isClassProxy = array_search('Neos\Form\Core\Runtime\FormRuntime', $classParents) !== false && array_search('Doctrine\ORM\Proxy\Proxy', $classImplements) !== false;
...
# PathAndFilename: /var/www/html/neos/Packages/Application/Neos.Form/Classes/Core/Runtime/FormRuntime.php
#