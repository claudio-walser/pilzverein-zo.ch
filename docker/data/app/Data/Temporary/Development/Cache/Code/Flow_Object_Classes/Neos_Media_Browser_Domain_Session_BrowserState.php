...
class BrowserState extends BrowserState_Original implements \Neos\Flow\ObjectManagement\Proxy\ProxyInterface {
    ...
    private function initializeData(string $assetSourceIdentifier)
    {
        ...
        if (isset($this->Flow_Aop_Proxy_methodIsInAdviceMode['initializeData'])) {
            $result = parent::initializeData($assetSourceIdentifier);

        } else {
            $this->Flow_Aop_Proxy_methodIsInAdviceMode['initializeData'] = true;
            try {
                ...
                $adviceChains = $this->Flow_Aop_Proxy_getAdviceChains('initializeData');
                $adviceChain = $adviceChains['Neos\Flow\Aop\Advice\AroundAdvice'];
                $adviceChain->rewind();
                $joinPoint = new \Neos\Flow\Aop\JoinPoint($this, 'Neos\Media\Browser\Domain\Session\BrowserState', 'initializeData', $methodArguments, $adviceChain);
                $result = $adviceChain->proceed($joinPoint);
                $methodArguments = $joinPoint->getMethodArguments();

            } catch (\Exception $exception) {
                unset($this->Flow_Aop_Proxy_methodIsInAdviceMode['initializeData']);
                throw $exception;
            }
            unset($this->Flow_Aop_Proxy_methodIsInAdviceMode['initializeData']);
        }
...
# PathAndFilename: /var/www/html/neos/Packages/Application/Neos.Media.Browser/Classes/Domain/Session/BrowserState.php
#